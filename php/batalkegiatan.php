...
<header class="header">
        ...
        <?php if (count($kegiatanDiikuti) > 0): ?>
            <?php foreach ($kegiatanDiikuti as $kegiatan): ?>
                <div class="article-card">
                    ...
                    <form method="POST" action="../php/cancel_event.php" onsubmit="return confirmCancellation(event)">
                        <input type="hidden" name="event_id" value="<?= $kegiatan['event_id']; ?>">
                        <button type="submit" class="tombol batal">Batalkan</button>
                    </form>
                </div>
            <?php endforeach; ?>
        <?php else: ?>
    ...
    <script>
        function confirmCancellation(event) {    
            return confirm("Apakah anda yakin akan membatalkan partisipasi?");
        }
    </script>
</body>
</html>
